...
{
	if (seat->philos[no - 1].action != (t_action) SLEEP)
		return ;
	seat->philos[no - 1].action = (t_action) THINK;
	printf("%ld %d is thinking\n", ms, no);
}

void	philo_eat(time_t ms, int no, t_seat *seat)
{
	if (seat->philos[no - 1].action != (t_action) THINK)
		return ;
	if (seat->philos[no - 1].forks != 2)
		return ;
	seat->philos[no - 1].action = (t_action) EAT;
	printf("%ld %d is eating\n", ms, no);
	usleep(seat->rules->eat * 1000);
	seat->philos[no - 1].forks -= 2; 
}

void	philo_sleep(time_t ms, int no, t_seat *seat)
{
	if (seat->philos[no - 1].action != (t_action) EAT)
		return ;
	seat->philos[no - 1].action = (t_action) SLEEP;
...
static void	philo_take(time_t ms, int no, t_seat *seat)
{
	...
	seat->philos[no - 1].forks++;
	printf("%ld %d has %d forks\n", ms, no, seat->philos[no - 1].forks);
}
