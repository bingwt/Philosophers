...
/*   Updated: 2024/04/28 04:48:23 by btan             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_rules	*r_init(int argc, char **argv)
{
	...
	rules->f_mutex = ft_calloc(no, sizeof(pthread_mutex_t));
	if (argc >= 6)
		rules->must_eat = ft_atol(argv[5]);
	else
		rules->must_eat = -1;
	while (no--)
		pthread_mutex_init(&rules->f_mutex[no], NULL);
	return (rules);
}
...
t_philo	*p_init(char **argv, t_rules *rules)
{
	...
	while (id--)
	{
		...
		pthread_mutex_init(&philo[id].p_mutex, NULL);
	}
	return (philo);
}
